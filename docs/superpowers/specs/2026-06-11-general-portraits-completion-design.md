...
游戏运行时，头像会与 `sanguobaye-web/public/assets/images/bg_world_map.jpg`、主菜单背景、城池图标以及暗金 UI 面板同屏展示，因此头像不应只作为独立插画成立，还需要与现有地图和界面资产保持视觉协调。
根据当前使用场景，头像至少会出现在右侧将领/城池信息面板的大尺寸头像框中，也会出现在命令反馈弹窗的小尺寸头像位置，因此同一张头像需要同时满足大图气质和小图可读性。
...
- 三国时期历史人物头像。
- 半身像，头肩构图，主体居中。
- 参考现有大地图 `bg_world_map.jpg` 的羊皮纸、赭石、暗金、烟褐色调，以及浮雕式古地图质感。
...
- 深色纯色背景。
- 人脸、头冠/发髻、肩部轮廓需要在右侧大头像框中有足够气势，也要在弹窗小头像中保持辨识度。
- 构图应保留安全边距，避免头冠、肩甲、胡须贴边或被裁切；人物头部和上半身比例要适合 1:1 裁切。
- 不出现文字、标志、水印、现代元素。
...
4. 重点检查：
   ...
   - 头像在右侧大面板和弹窗小头像两种场景下，是否都能看清人物身份特征。
...
- 已对代表性头像进行同屏风格检查，确认它们与现有大地图 `bg_world_map.jpg`、主菜单暗金战场氛围和 UI 面板色调协调。
- 已检查代表性头像在右侧大头像框和弹窗小头像尺寸下的可读性。
