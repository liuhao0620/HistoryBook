...
export const GameScreen: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (completedTasks.length > 0) {
                ...
                if (task.type === 'ATTACK') {
                    ...
                    if (defenderIds.length > 0) {
                        ...
                        if (task.forceId !== playerForceId && targetCityBelong !== playerForceId) {
                            ...
                            battlesToQueue.push({
                                fromCityId, targetCityId, executors, defenderIds, attackerForceId: task.forceId
                            });
                        }
                    }
                }
            ...
            if (newAiReports.length > 0) {
                setAiBattleReports(prev => [...prev, ...newAiReports]);
            }
            if (battlesToQueue.length > 0) {
                useGameStore.setState(state => ({ pendingPlayerBattles: [...state.pendingPlayerBattles, ...battlesToQueue] }));
            }
            processDelayedTasks();
        }
    }, [month]);

    const pendingPlayerBattles = useGameStore(state => state.pendingPlayerBattles);
    const [preBattleNote, setPreBattleNote] = useState<any | null>(null);
    ...
    useEffect(() => {
        if (pendingPlayerBattles.length > 0 && !preBattleNote && aiBattleReports.length === 0 && !showReportModal) {
            setPreBattleNote(pendingPlayerBattles[0]);
        }
    }, [pendingPlayerBattles, preBattleNote, aiBattleReports, showReportModal]);
    ...
    const handleCommandClick = (cmd: string) => {
        if (cmd === '赏赐' && (!currentCity?.toolQueue || currentCity.toolQueue.length === 0)) {
            alert('城中无闲置道具可赏赐。');
            return;
        }
    ...
    const handleSingleExecutorSelected = (generalId: number) => {
        ...
        } else if (['赏赐', '没收'].includes(cmd!)) {
            setCommandCtx(prev => ({ ...prev, step: 'SELECT_ITEM', executors: [generalId] }));
        } else {
    ...
    const executeCommands = (cmd: string, executors: number[], extraArgs?: any) => {
        ...
        for (const executorId of executors) {
            ...
            switch (cmd) {
                ...
                case '征兵': commandObj = new ConscriptionCommand(currentCity.id, executorId, extraArgs?.amounts?.arms || 0); break;
                ...
                case '分配': 
                    ...
                    break;
                case '赏赐': commandObj = new LargessCommand(currentCity.id, executorId, extraArgs?.targetItemId!); break;
                case '没收': commandObj = new ConfiscateCommand(currentCity.id, executorId, extraArgs?.targetItemId!); break;
            ...
            if (commandObj) {
                ...
                if (result.success) {
                    if (!['赏赐', '没收', '宴请', '处斩', '流放', '分配'].includes(cmd)) {
                        store.updatePerson(executorId, p => { p.acted = true; });
                    }
                    if (result.message) {
                        const avatarId = ['处斩', '流放'].includes(cmd) 
                            ? forces[playerForceId].kingId 
                            : (['赏赐', '没收', '宴请'].includes(cmd) ? (extraArgs?.targetPersonId || executorId) : executorId);
                        setReportMsg({ avatarId, text: result.message });
                    }
                } else {
                    if (result.message) {
                        const avatarId = ['处斩', '流放'].includes(cmd) ? forces[playerForceId].kingId : executorId;
                        setReportMsg({ avatarId, text: result.message });
                    }
                }
            }
    ...
    const renderCommandUI = () => {
        ...
        if (commandCtx.step === 'SELECT_EXECUTORS') {
            ...
            const availablePersons = getAvailableExecutors(
                commandCtx.cmd!, 
            ...
            return (
                <div style={{ position: 'absolute', inset: 0, zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    />
                    <div className="ancient-menu-panel" style={{ width: '800px', padding: '24px', position: 'relative', transform: `scale(${scale})`, transformOrigin: 'center center' }}>
                        <div style={{ paddingBottom: '16px', marginBottom: '16px', borderBottom: '2px solid #b78031', fontSize: '28px', color: '#f1c66f', fontWeight: 'bold' }}>
                            {`请选择【${commandCtx.cmd}】的${isTargetOnlyCmd ? '目标' : '执行武将'}`}
                        </div>
                        <div style={{ display: 'flex', borderBottom: '2px solid #b78031', padding: '8px 10px', fontSize: '22px', fontWeight: 'bold', color: '#d6a85b' }}>
                        ...
                        <div className="custom-scrollbar" style={{ maxHeight: '400px', overflowY: 'auto' }}>
                            {availablePersons.length === 0 ? (
                                <div style={{ padding: '32px', textAlign: 'center', fontSize: '24px', color: '#766244' }}>无可选择的武将。</div>
                                ...
                                return (
                                    <div 
                                        ...
                                        style={{ 
                                            ...
                                            cursor: 'pointer', borderBottom: '1px solid #766244',
                                    ...
                                    >
                                        ...
                                        <div style={{ flex: 1.5, fontSize: '18px', color: '#b78031' }}>{equipNames}</div>
                                        {commandCtx.cmd === '分配' && isSelected && (
                                            <div style={{ width: '100px' }}>
                                                <input 
                                                    ...
                                                    style={{ width: '80px', fontSize: '20px', backgroundColor: '#2a1609', border: '1px solid #b78031', padding: '4px 8px', color: '#f1c66f', fontFamily: '"Kaiti", "STKaiti", "KaiTi", "Songti SC", serif' }}
                                                />
                                            </div>
                                        )}
                                    </div>
                        ...
                        {isMulti && (
                            <div style={{ textAlign: 'center', paddingTop: '24px', display: 'flex', justifyContent: 'center' }}>
                                <button className="ancient-menu-button" onClick={handleMultiSelectConfirm}>确认</button>
                            </div>
                        )}
                    </div>
                </div>
        ...
        if (commandCtx.step === 'SELECT_TARGET_CITY') {
            return (
                <div style={{ position: 'absolute', top: '20px', left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(26,17,12,0.9)', padding: '15px 30px', zIndex: 200, color: 'var(--theme-gold)', border: '2px solid var(--theme-gold)', fontSize: '24px', textAlign: 'center', borderRadius: '8px' }}>
                    <div>请在大地图上点击目标城池。</div>
                </div>
            );
        }

        if (commandCtx.step === 'SELECT_TARGET_PERSON') {
            const availablePersons = getAvailableTargets(
                commandCtx.cmd!,
                Object.values(persons),
                currentCity?.id || 0,
                playerForceId,
            ...
            return (
                <GameModal 
                    ...
                    title="请选择目标武将"
                    style={{ width: '500px', transform: `scale(${scale})`, transformOrigin: 'center center' }}
                >
        ...
        if (commandCtx.step === 'INPUT_AMOUNTS') {
            const handleAmountChange = (key: string, value: string, max: number) => {
                ...
                if (num > max) num = max;
                if (num < 0) num = 0;
                setCommandCtx(prev => ({ ...prev, amounts: { ...prev.amounts, [key]: num } }));
            };

            if (commandCtx.cmd === '输送') {
                return (
                    <GameModal 
                        isOpen={true} 
                        onClose={() => setCommandCtx({ cmd: null, step: 'NONE', executors: [], amounts: {} })}
                        title="输入输送数量"
                        style={{ width: '400px', transform: `scale(${scale})`, transformOrigin: 'center center' }}
                    >
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px' }}>
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <span>金钱 (最大 {currentCity?.money}):</span>
                                    <input type="number" min="0" max={currentCity?.money} value={commandCtx.amounts?.money || 0} onChange={e => handleAmountChange('money', e.target.value, currentCity?.money || 0)} style={{ width: '100px', fontSize: '20px', padding: '2px 5px', backgroundColor: '#2a1609', border: '1px solid #b78031', color: '#f1c66f' }} />
                                </div>
                                <input type="range" min="0" max={currentCity?.money || 0} value={commandCtx.amounts?.money || 0} onChange={e => handleAmountChange('money', e.target.value, currentCity?.money || 0)} style={{ width: '100%' }} />
                            </div>
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <span>粮食 (最大 {currentCity?.food}):</span>
                                    <input type="number" min="0" max={currentCity?.food} value={commandCtx.amounts?.food || 0} onChange={e => handleAmountChange('food', e.target.value, currentCity?.food || 0)} style={{ width: '100px', fontSize: '20px', padding: '2px 5px', backgroundColor: '#2a1609', border: '1px solid #b78031', color: '#f1c66f' }} />
                                </div>
                                <input type="range" min="0" max={currentCity?.food || 0} value={commandCtx.amounts?.food || 0} onChange={e => handleAmountChange('food', e.target.value, currentCity?.food || 0)} style={{ width: '100%' }} />
                            </div>
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <span>兵力 (最大 {currentCity?.mothballArms}):</span>
                                    <input type="number" min="0" max={currentCity?.mothballArms} value={commandCtx.amounts?.arms || 0} onChange={e => handleAmountChange('arms', e.target.value, currentCity?.mothballArms || 0)} style={{ width: '100px', fontSize: '20px', padding: '2px 5px', backgroundColor: '#2a1609', border: '1px solid #b78031', color: '#f1c66f' }} />
                                </div>
                                <input type="range" min="0" max={currentCity?.mothballArms || 0} value={commandCtx.amounts?.arms || 0} onChange={e => handleAmountChange('arms', e.target.value, currentCity?.mothballArms || 0)} style={{ width: '100%' }} />
                            </div>
                        </div>
            ...
            if (commandCtx.cmd === '交易') {
                const isBuy = (commandCtx.amounts?.type as any) !== 'sell';
                const maxAmount = isBuy ? (currentCity?.money || 0) * 5 : (currentCity?.food || 0);
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '20px' }}>
                            <label style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer' }}>
                                <input type="radio" checked={isBuy} onChange={() => setCommandCtx(prev => ({ ...prev, amounts: { type: 'buy', amount: 0 } }))} /> 买入粮食 (1金=5粮)
                            </label>
                            <label style={{ display: 'flex', alignItems: 'center', gap: '5px', cursor: 'pointer' }}>
                                <input type="radio" checked={!isBuy} onChange={() => setCommandCtx(prev => ({ ...prev, amounts: { type: 'sell', amount: 0 } }))} /> 卖出粮食 (1粮=2金)
                            </label>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px' }}>
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <span>{isBuy ? '买入数量' : '卖出数量'} (最大 {maxAmount}):</span>
                                    <input type="number" min="0" max={maxAmount} value={commandCtx.amounts?.amount || 0} onChange={e => handleAmountChange('amount', e.target.value, maxAmount)} style={{ width: '100px', fontSize: '20px', padding: '2px 5px', backgroundColor: '#2a1609', border: '1px solid #b78031', color: '#f1c66f' }} />
                                </div>
                                <input type="range" min="0" max={maxAmount} value={commandCtx.amounts?.amount || 0} onChange={e => handleAmountChange('amount', e.target.value, maxAmount)} style={{ width: '100%' }} />
                            </div>
                            <div style={{ textAlign: 'center', color: '#b78031' }}>
                                {isBuy 
                                    ? `将花费 ${Math.ceil((commandCtx.amounts?.amount as number || 0) / 5)} 金钱` 
                                    : `将获得 ${(commandCtx.amounts?.amount as number || 0) * 2} 金钱`}
                            </div>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <GameButton onClick={() => executeCommands(commandCtx.cmd!, commandCtx.executors, { amounts: commandCtx.amounts })}>确认交易</GameButton>
                        </div>
                    </GameModal>
                );
            }
            ...
            if (commandCtx.cmd === '征兵') {
                ...
                return (
                    <GameModal 
                        ...
                        style={{ width: '400px', transform: `scale(${scale})`, transformOrigin: 'center center' }}
                    >
                        <div style={{ marginBottom: '15px', color: '#b78031', fontSize: '18px' }}>
                            <div>城池民忠: {currentCity?.peopleDevotion} (最大征兵 {maxByDevotion})</div>
                            <div>城池金钱: {currentCity?.money} (最大征兵 {maxByMoney})</div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px' }}>
                            <div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                    <span>征兵数量 (最大 {maxAmount}):</span>
                                    <input type="number" min="0" max={maxAmount} value={commandCtx.amounts?.arms || 0} onChange={e => handleAmountChange('arms', e.target.value, maxAmount)} style={{ width: '100px', fontSize: '20px', padding: '2px 5px', backgroundColor: '#2a1609', border: '1px solid #b78031', color: '#f1c66f' }} />
                                </div>
                                <input type="range" min="0" max={maxAmount} value={commandCtx.amounts?.arms || 0} onChange={e => handleAmountChange('arms', e.target.value, maxAmount)} style={{ width: '100%' }} />
                            </div>
                            <div style={{ textAlign: 'center', color: '#b78031' }}>
                                将花费 {Math.floor((commandCtx.amounts?.arms as number || 0) / 2)} 金钱
                            </div>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'center' }}>
                            <GameButton onClick={() => executeCommands(commandCtx.cmd!, commandCtx.executors, { amounts: commandCtx.amounts })}>确认征兵</GameButton>
                        </div>
                    </GameModal>
                );
            }
        ...
        if (commandCtx.step === 'SELECT_ITEM') {
            const executor = persons[commandCtx.executors[0]];
            if (!executor) return null;
            
            let availableItems: { id: number, name: string, desc: string }[] = [];
            if (commandCtx.cmd === '没收') {
                availableItems = (executor.equip || []).map(gid => {
                    const g = useGameStore.getState().goods[gid];
                    return { id: gid, name: g?.name || '未知', desc: `价值 ${g?.money || 0}` };
                });
            } else if (commandCtx.cmd === '赏赐') {
                availableItems = (currentCity?.toolQueue || []).map(gid => {
                    const g = useGameStore.getState().goods[gid];
                    return { id: gid, name: g?.name || '未知', desc: `价值 ${g?.money || 0}` };
                });
            }
        ...
        <div style={{ display: 'flex', height: '100%', width: '100%', backgroundColor: '#20150d', color: '#d6a85b', fontFamily: '"STKaiti", "KaiTi", serif', alignItems: 'center', justifyContent: 'center', userSelect: 'none' }}>
            ...
            <div style={{ display: 'flex', width: '100%', height: '100%', backgroundColor: '#25180d' }}>
                ...
                <div style={{ flex: 1, position: 'relative', borderRight: `${2 * scale}px solid #b78031`, boxSizing: 'border-box', backgroundColor: '#25180d', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'inset 0 0 90px rgba(0,0,0,0.45)' }}>
                    ...
                    <div 
                        onClick={(e) => {
                            if (aiThinkingForceId !== null) return; // 回合结算阶段禁止呼出系统菜单
                            ...
                            <>
                                ...
                                <div className="ancient-menu-panel" style={{ 
                                    ...
                                    {menuState === 'SAVE' && (
                                        <div style={{ display: 'flex', flexDirection: 'column', width: '360px' }}>
                                            ...
                                            {saveSlots.map((s, idx) => (
                                                ...
                                                >
                                                    <span style={{ fontSize: '24px' }}>进度 {s.slot}</span>
                                    ...
                                    {menuState === 'DIPLOMACY' && (
                                        <div style={{ display: 'flex', flexDirection: 'column', width: '200px' }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                ...
                                                <button className="ancient-menu-button" style={{ borderBottom: '1px solid rgba(183, 128, 49, 0.3)' }} onClick={() => handleCommandClick('反间')}>反间</button>
                                    ...
                                    {menuState === 'STATUS' && currentCity && (
                                        <div style={{ display: 'flex', flexDirection: 'column', width: '280px', fontSize: '22px', padding: '0 24px' }}>
                    ...
                    {reportMsg && (
                        <GameModal 
                            ...
                            style={{ width: '600px', cursor: 'pointer', transform: `scale(${scale})`, transformOrigin: 'center center' }}
                        >
                    ...
                    {showReportModal && (
                        <GameModal 
                            ...
                            title="回合汇报"
                            style={{ width: '600px', transform: `scale(${scale})`, transformOrigin: 'center center' }}
                        >
                    ...
                    {preBattleNote && (
                        <GameModal
                            isOpen={true}
                            onClose={() => {
                                const { fromCityId, targetCityId, executors, defenderIds } = preBattleNote;
                                useGameStore.setState(state => ({ pendingPlayerBattles: state.pendingPlayerBattles.slice(1) }));
                                setPreBattleNote(null);
                        ...
                        >
                            <div style={{ fontSize: '22px', color: 'var(--theme-dark)', lineHeight: '1.6', minHeight: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center', whiteSpace: 'pre-line' }}>
                                {preBattleNote.attackerForceId === playerForceId 
                                    ? `我军向 ${cities[preBattleNote.targetCityId]?.name || '敌城'} 发起了进攻！` 
                                    : `【军情急报】${forces[preBattleNote.attackerForceId] ? persons[forces[preBattleNote.attackerForceId].kingId]?.name : '敌'} 军正在攻打我方 ${cities[preBattleNote.targetCityId]?.name || '城池'}！请主公准备迎战！`}
                            </div>
                            <div style={{ textAlign: 'center', marginTop: '20px' }}>
                                <GameButton onClick={() => {
                                    const { fromCityId, targetCityId, executors, defenderIds } = preBattleNote;
